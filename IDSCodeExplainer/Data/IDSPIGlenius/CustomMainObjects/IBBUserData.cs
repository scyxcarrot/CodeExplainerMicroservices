...
namespace IDS.Common.ImplantBuildingBlocks
{
    ...
    public class IBBUserData : Rhino.DocObjects.Custom.UserData
    {
        ...
        private static int ComputeCharacteristicValue(GeometryBase geom)
        {
            if (geom is Mesh)
            {
                var am = AreaMassProperties.Compute((Mesh)geom);
                var crc = RhinoMath.CRC32((uint)am.Area, am.Centroid.X);
            ...
            if (geom is Brep)
            {
                var am = AreaMassProperties.Compute((Brep)geom);
                var crc = RhinoMath.CRC32((uint)am.Area, am.Centroid.X);
                crc = RhinoMath.CRC32(crc, am.Centroid.Y);
                crc = RhinoMath.CRC32(crc, am.Centroid.Z);
                return (int)crc;
            ...
            if (geom is Surface)
            {
                var am = AreaMassProperties.Compute((Surface)geom);
                var crc = RhinoMath.CRC32((uint)am.Area, am.Centroid.X);
                crc = RhinoMath.CRC32(crc, am.Centroid.Y);
                crc = RhinoMath.CRC32(crc, am.Centroid.Z);
                return (int)crc;
            ...
            if (geom is Curve)
            {
                var curve = (Curve)geom;
                var centroid = curve.ComputeCentroid(0.5);
                var crc = RhinoMath.CRC32((uint)curve.SpanCount, curve.Degree);
