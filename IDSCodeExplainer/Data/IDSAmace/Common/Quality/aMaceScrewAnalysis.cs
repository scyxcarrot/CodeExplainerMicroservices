...
{
    public class AmaceScrewAnalysis : Common.Quality.ScrewAnalysis
    {
        /// <summary>
        ...
        public void PerformSlowScrewChecks(List<Screw> screws, Mesh plateBumps, Cup cup,
        ...
        /// <summary>
        /// Performs the guide hole boolean intersection check with guide hole safety zone
        /// </summary>
        /// <param name="screws">The screws.</param>
        /// <param name="drillBitRadius">The screws.</param>
        /// <returns></returns>
        public Dictionary<int, List<int>> PerformGuideHoleBooleanIntersectionCheck(List<Screw> screws, double drillBitRadius)
        {
            // Init dict, key = sourceScrew, value = list of problematic targetScrews
            var intersections = new Dictionary<int, List<int>>();

            var guideHoleBooleans = new Dictionary<int, Brep>();
            var guideHoleSafetyZones = new Dictionary<int, Brep>();
            foreach (var screw in screws)
            {
                Brep guideHoleBoolean;
                Brep guideHoleSafetyZone;
                GetGuideHoleBreps(screw, drillBitRadius, out guideHoleBoolean, out guideHoleSafetyZone);
                guideHoleBooleans.Add(screw.Index, guideHoleBoolean);
                guideHoleSafetyZones.Add(screw.Index, guideHoleSafetyZone);
            }

            // Loop over sourceScrews and targetScrews
            foreach (var sourceScrew in screws)
            {
                foreach (var targetScrew in screws)
                {
                    if (sourceScrew == targetScrew)
                    {
                        continue;
                    }

                    var guideHoleBoolean = guideHoleBooleans[sourceScrew.Index];
                    var guideHoleSafetyZone = guideHoleSafetyZones[targetScrew.Index];

                    var isIntersected = PerformGuideHoleBooleanIntersectionCheck(guideHoleBoolean, guideHoleSafetyZone);
                    if (isIntersected)
                    {
