...
namespace IDS.Amace.FileSystem
{
    /// <summary>
    ...
    public static class DirectoryStructure
    {
            ...
            List<string> directoryExceptions, List<string> fileExceptions, List<string> extensions, List<string> directoryToDeleteIfExist)
        {
            ...
            {
                Dialogs.ShowMessageBox(string.Format("The work file is not in a folder called Work. Viewing " +
            ...
            {
                Dialogs.ShowMessageBox(string.Format("The parent directory does not contain the input " +
            ...
            foreach (string dir in directoryToDeleteIfExist)
            {
                ...
                DialogResult deleteExistingOutputDialogResult = Dialogs.ShowMessageBox
                    ($"A {dir} folder already exists and will be deleted. Is this OK?",
                    $"{dir} folder exists", MessageBoxButtons.YesNo, MessageBoxIcon.Exclamation);
        ...
        public static bool CheckDirectoryIntegrity(string rootDirectory, List<string> directoryExceptions, List<string> fileExceptions, List<string> extensions)
        {
            ...
            foreach (string directory in Directory.EnumerateDirectories(rootDirectory))
            {
                ...
                {
                    Dialogs.ShowMessageBox(string.Format("The target directory contains subfolders{0}. Please use a clean folder.", directoryExceptionString),
            ...
            foreach (string ext in extensions)
            {
                foreach (string file in Directory.EnumerateFiles(rootDirectory, ext))
                {
                    ...
                    Dialogs.ShowMessageBox(
                        $"The target directory contains{extensionString}files{fileExceptionString}. Please use a clean folder.",
        ...
        /// <param name="director">The director.</param>
        /// <returns></returns>
        public static string GetGuideOutputFolder(IImplantDirector director)
        {
            string folder = MakeExportDir("IDS_For_Guide", director, true);
            return folder;
        }
        ...
        /// <param name="director">The director.</param>
        /// <returns></returns>
        public static string GetReportingOutputFolder(IImplantDirector director)
        {
            string folder = MakeExportDir("IDS_For_Reporting", director, true);
            return folder;
        }
        ...
        /// <param name="director">The director.</param>
        /// <returns></returns>
        public static string GetFinalizationOutputFolder(IImplantDirector director)
        {
            string folder = MakeExportDir("IDS_For_Finalization", director, true);
            return folder;
        }
        ...
        /// <param name="director">The director.</param>
        /// <returns></returns>
        public static string GetPlasticProdOutputFolder(IImplantDirector director)
        {
            string folder = MakeExportDir("IDS_For_Plastic_Models", director, true);
            return folder;
        }

        /// <summary>
        /// Gets the virtual bench test output folder.
        /// </summary>
        /// <param name="director">The director.</param>
        /// <returns></returns>
        public static string GetVirtualBenchTestOutputFolder(IImplantDirector director)
        {
            string folder = MakeExportDir("IDS_Virtual_Bench_Test", director, true);
            return folder;
        }
        ...
        /// <param name="director">The director.</param>
        /// <param name="addCaseIdPrefix">The addCaseIdPrefix.</param>
        /// <returns></returns>
        private static string MakeExportDir(string inputFolderName, IImplantDirector director, bool addCaseIdPrefix)
        {
            // Get output dir
            string outputDir = GetOutputFolderPath(director.Document);
            var folderName = inputFolderName;

            if (addCaseIdPrefix)
            {
                folderName = $"{director.caseId}_{folderName}";
            }

            // Get path to outputdir
            string exportDir = Path.Combine(outputDir, folderName);
