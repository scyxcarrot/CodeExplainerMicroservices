...
namespace IDS.Core.NonProduction
{
    ...
    {
#if (INTERNAL)
        public static void HandleStopWatch(Stopwatch stopWatch, string subject, bool restartTimer = false)
        {
            stopWatch.Stop();
            IDSPluginHelper.WriteLine(LogCategory.Default,
                subject +
                $"{ (stopWatch.ElapsedMilliseconds * 0.001).ToString(CultureInfo.InvariantCulture) } seconds");
            stopWatch.Reset();
        ...
        public static Guid AddObject(Mesh obj, string objName, string layerName, bool replaceIfExists = false)
        {
            return AddObject(Guid.Empty, obj, objName, layerName, replaceIfExists);
        }

        public static Guid AddObject(Guid id, Mesh obj, string objName, string layerName, bool replaceIfExists = false)
        {
            if (obj == null)
            {
                return Guid.Empty;
        ...
        public static Guid ReplaceObject(Mesh obj, string layerName)
        {
            return AddObject(obj, layerName, layerName, true);
        }
        ...
        public static Guid AddPointMesh(Point3d pt, string name)
        {
            var sphere = new Sphere(pt, 0.1);
            var sphMesh = Mesh.CreateFromSphere(sphere, 100, 100);

            return AddObject(sphMesh, name);
        }
        ...
        public static Guid AddCurveMesh(Curve curve, string objName)
        {
            if (curve == null)
            {
                return Guid.Empty;
            ...
            Mesh tube;
            if (!TubeFromPolyline.PerformMeshFromPolyline(curve, 0.05, out tube))
            {
                throw new IDSException("Create Patch: Tube Creation Failed!");
            }

            return AddObject(tube, objName);
        }

        public static Guid AddCurve(Curve curve, string layerName, Color color)
        {
            return AddCurve(curve, layerName, layerName, color);
        }
        ...
        public static void ExportPlaneXml(Plane plane, string name, string path, double planeSize = 50.0)
        {
            ...
            nodeEntities.AppendChild(CreatePlaneNode(xmlDoc, plane, name, planeSize));

            xmlDoc.Save(path + name + ".xml");
        }

        private static XmlNode CreatePlaneNode(XmlDocument xmlDoc, Plane plane, string name, double planeSize = 50.0)
        {
