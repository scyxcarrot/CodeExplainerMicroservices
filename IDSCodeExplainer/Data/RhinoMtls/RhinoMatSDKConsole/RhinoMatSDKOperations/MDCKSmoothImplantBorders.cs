...
namespace RhinoMatSDKOperations.Smooth
{
    ...
    {
        public static bool OperatorSmoothEdge(string topMesh, string sideMesh, string bottomMesh, string roundedModelPath, MDCKSmoothImplantBorderParameters opParams)
        {
            return OperatorSmoothEdge(topMesh, sideMesh, bottomMesh, roundedModelPath, opParams, false);
        }

        public static bool OperatorSmoothEdge(string topPath, string sidePath, string bottomPath, string roundedModelPath, MDCKSmoothImplantBorderParameters opParams, bool applyFixes)
        {
            // Import the 3 stls as 3 models
            MDCK.Model.Objects.Model model1;
            var success = MDCKInputOutput.ModelFromStlPath(topPath, out model1);
            File.Delete(topPath);
            if (!success)
            {
                return false;
            ...
            if (!MDCKConversion.ExportMDCK2StlFile(model, roundedModelPath))
            {
                return false;
            }
